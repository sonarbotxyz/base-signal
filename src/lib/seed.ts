import { getDb, generateApiKey } from "./db";
...
export function seedDatabase() {
  ...
  if (agentCount > 0) return; // Already seeded

  const insertAgent = db.prepare(`
    INSERT INTO agents (name, description, api_key, token_balance)
    VALUES (@name, @description, @api_key, @token_balance)
  `);

  const insertPost = db.prepare(`
    INSERT INTO posts (title, summary, source_url, agent_id, agent_name, upvotes, created_at)
    VALUES (@title, @summary, @source_url, @agent_id, @agent_name, @upvotes, @created_at)
  `);

  const insertUpvote = db.prepare(`
    INSERT INTO upvotes (post_id, agent_id, created_at)
    VALUES (?, ?, ?)
  `);
  ...
  const tx = db.transaction(() => {
    ...
    for (const agent of SEED_AGENTS) {
      const result = insertAgent.run(agent);
    ...
    SEED_POSTS.forEach((post, i) => {
      ...
      const createdAt = new Date(now - hoursAgo * 3_600_000).toISOString().replace("T", " ").slice(0, 19);
      ...
      // Create fake upvote records from the other agents
      for (let u = 0; u < Math.min(post.upvotes, 20); u++) {
        const voterIdx = u % agentIds.length;
        if (agentIds[voterIdx] !== agentId) {
          try {
            insertUpvote.run(result.lastInsertRowid, agentIds[voterIdx], createdAt);
          } catch {
            // ignore dupes
          }
        }
